...
const ThoughtItem = ({ thought, setThoughts, thoughts }) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/thoughts/${thought.id}`, {
        method: 'PUT',
        credentials: 'include',
      ...
      if (res.ok) {
        setThoughts(thoughts.map(t => t.id === thought.id ? { ...t, text: editText } : t));
...
const AllThoughts = ({ thoughts, setThoughts }) => {
  return (

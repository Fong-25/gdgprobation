		...
		<div className="max-w-7xl mx-auto mt-4 sm:mt-6">
			<form className="flex flex-col sm:flex-row gap-2 sm:gap-3" onSubmit={handleSubmit}>
				<input
					...
					className="flex-1 w-full px-3 py-2 text-sm sm:text-base border border-violet-400 rounded"
					disabled={submitting || !canAddToday}
				/>

				<button
					type="submit"
					className={`px-4 py-2 text-sm sm:text-base border border-violet-400 rounded whitespace-nowrap ${submitting || !canAddToday ? 'bg-violet-800 text-gray-500' : 'bg-indigo-950 text-white hover:opacity-90'}`}
					disabled={submitting || !canAddToday}
				>
